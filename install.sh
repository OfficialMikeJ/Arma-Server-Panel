...
configure() {
  ...
  # The API container runs unprivileged, so it must join the host's docker
  # group to use the socket. This gid is not the same everywhere.
  _docker_gid=$(stat -c '%g' /var/run/docker.sock 2>/dev/null || echo 999)
  ok "Docker socket group id: ${_docker_gid}"
